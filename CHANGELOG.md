...
### macOS test build (the 2026-08-28 field-test fixes)

Five fixes for the Mac field test, assembled onto this build. The Windows CPU ladder, model table and recording format work above are unchanged.

- **Stop always finishes now, even on a busy Mac.** A session on a contended graphics chip used to leave Stop spinning forever with nothing but the word "Finishing" and no way out but force-quitting the window. Stop is now bounded: it finishes the backlog when it can, and when the machine is genuinely stuck it says so and offers "Keep waiting" or "Stop now", keeping everything transcribed so far and finalising the recording in full. The "Finishing" text now shows a real count (or an honest "still starting up") instead of a bare, countless spinner, including when Stop is pressed while the model is still catching up. (`live_transcribe/web/app.py`, `live_transcribe/web/static/app.js`.)
- **A struggling Mac now says so.** On Apple graphics there is no model ladder to step down, so a Mac that could not keep up used to drop audio silently. A backend-agnostic "falling behind" warning now watches the transcription queue on any backend (GPU, MLX or CPU) and raises the same banner, so a busy graphics chip is flagged rather than quietly losing audio. (`live_transcribe/transcribe.py`, `live_transcribe/web/app.py`, `live_transcribe/web/static/app.js`.)
- **Transcription failures are no longer invisible.** A backend that throws on every chunk (a broken install, an out-of-memory graphics call) used to look exactly like a quiet room: an empty transcript and a calm "Listening" chip. Per-chunk failures are now counted, and past a small threshold one banner says transcription is failing and names the log file to send when asking for help. (`live_transcribe/transcribe.py`, `live_transcribe/web/app.py`, `live_transcribe/web/static/app.js`.)
- **A second launch no longer opens a second copy.** The app claims a single fixed port on startup and hands off to the running copy, so double-clicking again brings the existing window forward instead of starting a rival instance. (`live_transcribe/desktop.py`, `volksmond-mac.spec`.)
- **The Mac app wears the Volksmond icon.** The macOS .icns is generated from the brand mark at build time (the same mark as the Windows icon, white on brand blue) with a CI check that a build shipping the generic icon fails loudly. (`build-icon.py`, `mac/build-app-mac.sh`, `.github/workflows/mac-release.yml`.)
...
- Fix: a voice model downloaded earlier as a symlinked HuggingFace cache (Developer Mode machines) is now migrated to real files (a hardlink to the shared model blob, on Windows only) the first time the Models page checks it, so it reads as installed, loads in the packaged app, and shows its true size instead of a doubled one. The shared blob is kept, so any other cached revision of the model keeps working.
...
This release polishes the live-meeting experience and adds a Microsoft Store edition of Volksmond.

- **The meeting title and notes no longer lose your cursor while the model warms up.** The "preparing model" check used to redraw the whole screen every second and a half, which stole focus from whatever you were typing; it now redraws only when the warm-up state changes, and every text box keeps its cursor position across a redraw. (`web/static/app.js`.)
- **The always-applied context is now editable for a single meeting.** The context box on the start screen can be edited in place, and what you type is used for that one session only, replacing your saved default without changing it. Leave it untouched and the Settings default still applies. (`web/static/app.js`, `web/app.py`.)
- **On the Auto quality setting, Volksmond now uses the largest model size you have already downloaded** for that language family, within what your hardware can run, instead of picking a smaller size or re-downloading. If nothing is downloaded yet, it falls back to the size your hardware suggests. (`live_transcribe/__main__.py`.)
- **Volksmond now tells you when your computer cannot keep up, and lets you start a recording partway through a meeting.** On CPU, if transcription starts falling behind real time, a one-time banner and a Windows notification suggest recording now and transcribing later on a faster machine; you can switch this off in Settings. A "Record from here" button starts saving a recording mid-session, so you can still capture the rest of a meeting you did not record from the start. (`web/app.py`, `web/static/app.js`.)
- **A third build edition for the Microsoft Store, plus the MSIX packaging lane to ship it.** The Store edition is the normal connected app with exactly one network path compiled out: DigiPhyte's app-update manifest check, because the Store owns updates for its installs (the model-update check, the calendar and the licensing UI all stay). Its user-initiated update button now opens Volksmond's own Microsoft Store listing using public Store ID `9P7BD97WTZ3W`; Microsoft then shows Update or Open, and Volksmond makes no update request itself. Built like the offline edition, from the same spec, via a `VOLKSMOND_STORE=1` profile and a runtime hook that hard-sets the flag in the frozen app; `build-app.ps1 -Editions store` builds it, and the new `msix\build-msix.ps1` renders the Store tile assets from the brand mark, stamps the manifest with the Partner Center identity (kept out of this public repo) and packs an unsigned `.msix` with the Windows SDK's makeappx, ready for submission (the Store signs on ingestion). (`buildflags.py`, `pyi_rth_store.py` new, `sa-live-transcribe.spec`, `build-app.ps1`, `msix/` new, `web/app.py`, `web/static/app.js`.)
