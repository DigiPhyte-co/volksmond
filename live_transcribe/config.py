...
DEFAULTS = {
    ...
    "record_sessions": True,          # save the meeting audio on every live session. ON by default: when a live transcript collapses (a slow CPU dropping to a smaller model, a model that never loaded, the wrong microphone), the recording is the only way to get the meeting back, and re-transcribing the file beats guessing. The audio is written to the save location on THIS computer and never leaves it; the live screen says it is recording, and the finish screen offers a one-click delete. An install with no "record_sessions" key has never chosen, so it records; an explicit False is the user's own choice and is honoured for good.
    "recording_format": "flac",      # recording file format: "flac" (DEFAULT, about 115 MB/hour), "opus" (about 15 MB/hour), or "wav" (about 230 MB/hour). FLAC keeps the current 16-bit audio losslessly; Opus is the compact speech option. WAV is only the escape hatch if libsndfile ever fails, so a broken codec DLL never costs the meeting.
    ...
    "struggle_nudge": True,           # surface (banner + Windows notification) a live session that cannot transcribe in real time: the CPU auto-downgrade, and a GPU session falling behind because something else on the machine is using the card. The downgrade ALWAYS happens regardless; this only gates the SURFACING, so a user who knowingly runs a weak CPU or a shared GPU can silence it. Not a Business feature; data integrity, like silence_nudge. Hard kill: SA_LIVE_STRUGGLE_NUDGE=0.
}
