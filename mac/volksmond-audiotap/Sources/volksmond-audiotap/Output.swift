...
final class StdoutWriter {
    ...
    // stop: flush intent is moot, so exit 0 rather than crash on SIGPIPE.
    ...
    private func writeAllRawLocked(_ base: UnsafeRawPointer, _ count: Int) {
        ...
        while offset < count {
            ...
            if n < 0 {
                ...
                if err == EPIPE {
                    // Consumer closed the read end. Nothing left to do; leave
                    // cleanly. SIGPIPE itself is ignored in main() so we reach here.
                    exit(ExitCode.cleanStop.rawValue)
                }
                // Any other write error is unrecoverable for a streaming helper.
