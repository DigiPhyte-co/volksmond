...
#
# Two editions ship from one spec (docs/distribution-and-landing-plan.md section 3):
...
# The default builds BOTH. Pass -Editions connected for a fast dev build of just the
# normal app.
#
...
foreach ($edition in $Editions) {
    ...
    # The spec reads VOLKSMOND_OFFLINE to pick the profile. Scope it to this pass and always clear
    # it afterwards, so a connected pass can never inherit an offline flag from a prior one.
    ...
    # and app version (e.g. volksmond_1_10_0.zip / volksmond-offline_1_10_0.zip) so a tester can
    # see at a glance which build and edition they have.
    $zipBase = if ($offline) { "volksmond-offline_" } else { "volksmond_" }
    ...
    # plan; the offline zip is its deliverable for now.
    if ($offline) {
